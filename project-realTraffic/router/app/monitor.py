...
def extract_packet_info(ip_packet, transport_packet, proto):
    try:
        ...
        protocol_dummies = pd.get_dummies(packet_info_prediction['PROTOCOL_MAP'], prefix='PROTOCOL_MAP', drop_first=True)
        ...
        packet_info_prediction = packet_info_prediction.drop(['PROTOCOL_MAP','SERVICE','ANOMALY'], axis=1)  # Rimuove le colonne testuali
        packet_info_prediction = pd.concat([packet_info_prediction, protocol_dummies], axis=1)
        ...
        fitter = scaler.fit(packet_info_prediction)
...
def packet_statistics():
    ...
    icmp_count = sum(1 for packet in received_packets if packet["PROTOCOL_MAP"] == "icmp")
